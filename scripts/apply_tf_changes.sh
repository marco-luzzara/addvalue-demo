...
main() {
    ...
    docker exec "$TERRAFORM_CONTAINER_NAME" terraform apply \
        -auto-approve \
        -var="fill_lambda_dist_bucket=hot-reload" \
        -var="fill_lambda_dist_bucket_key=$(pwd)/../build/hot-reload" \
        -var="process_lambda_dist_bucket=hot-reload" \
        -var="process_lambda_dist_bucket_key=$(pwd)/../build/hot-reload"
    print_done
